...
8. **Bitrate-matched x265 comparison** — Defensible comparison still benefits from **two-pass** or iterative tightening when chasing a bitrate lock. **`bench_srsv2 --match-x265-bitrate`** (**`--x265-bitrate-sweep`** with a **CRF** ladder toward SRSV2’s bitrate, or without it a single **average-bitrate** FFmpeg attempt) selects the closest achieved rate and emits **`x265_sweep_rows`** — a **narrower helper** than full two-pass tuning, but **beyond CRF‑only guessing**. **`--compare-x265`** stays a convenience **fixed‑CRF** row.
