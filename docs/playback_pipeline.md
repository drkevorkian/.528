...
- **Decoders:** primary video by `codec_id`: **`codec_id` 1** → SRSV1 legacy grayscale intra (`libsrs_video::decode_frame`). **`codec_id` 3** → SRSV2 exposes **two decode entry points**: **`decode_yuv420_srsv2_payload`** — legacy **single-slot** helper sufficient for **`FR2`** rev **1**–**9** when callers manage one reconstructed picture themselves; **`decode_yuv420_srsv2_payload_managed`** — **bounded** **`SrsV2ReferenceManager`** path for rev **1**–**13** (intra **`FR2\x01`** / **`FR2\x03`** / **`FR2\x07`**, experimental **P** **`FR2\x02`**…**`\x09`**, experimental **B** **`FR2\x0A`**/**`\x0B`**/**`\x0D`**, experimental **alt-ref** **`FR2\x0C`**). **Playback** uses **only** the managed decoder for SRSV2. **B** requires **`max_ref_frames ≥ 2`** and both temporal anchors resident **before** the **B** packet (streams often store **decode order** *I₀ → P₂ → B₁*, not presentation order). **`PlaybackSession`** still surfaces decoded **display** frames **in decode bitstream order** — there is **no** presentation reorder buffer in the player path yet (`bench_srsv2` uses **`frame_index`** to score metrics in display order). **`FR2\x0A`/`\x0B`/`\x0D`** with **`max_ref_frames < 2`** → **`PlaybackError::Unsupported`**. **Alt-ref** **`FR2\x0C`** updates references with **`is_displayable == false`** (no on-screen texture advance). **`codec_id` 2** → SRSA (`libsrs_audio::decode_frame_with_stream_version`, v2 stream payloads).
...
| **3** | Video — **SRSV2** default | Intra YUV420p8 (`FR2\x01` / `FR2\x03` / `FR2\x07`); experimental **P** (`FR2\x02`…`\x09`); experimental **B** (`FR2\x0A`/`\x0B`/`\x0D`) when **`max_ref_frames ≥ 2`** + decode-order refs; experimental **alt-ref** (`FR2\x0C`, non-display); **B** + **`max_ref_frames < 2`** → unsupported |
