...
**`SrsV2EncodeSettings::subpel_refinement_radius`** is clamped ( **`0`** skips refinement; **`1`** runs the default eight probes). **Quarter-pel** motion beyond the current half-pel grid is not implemented. **B-frame** motion in **P**/**bench** tooling is not implemented; **experimental B** syntax (`FR2\x0A`/`\x0B`) decodes in **`libsrs_video`** and **`PlaybackSession`** when **`max_ref_frames ≥ 2`** (typically **decode-order** *I₀→P₂→B₁*); **`bench_srsv2 --bframes > 0`** remains unsupported. **GPU** motion search is not implemented.
