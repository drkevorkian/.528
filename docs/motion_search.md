...
**Variable P-frame inter partitions (`FR2` rev **19**/**20**, encoder `SrsV2InterPartitionMode`, default `Fixed16x16`):** ME runs per **partition unit** (**16×16**, **16×8**, **8×16**, or **8×8** regions — **no** unbounded quadtree). **`AutoFast`** compares a **small bounded** candidate set with deterministic λ-style side-information penalties; **not** production **RDO**. Requires **compact** or **entropy** inter syntax — **not** raw MV tuples. **`bench_srsv2`** exposes **`--inter-partition`**, **`--transform-size`**, and **`--compare-partitions`**.
