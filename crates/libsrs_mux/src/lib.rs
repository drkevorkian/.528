...
use libsrs_container::{
    ...
    PacketFlags, PacketHeader, TrackDescriptor, MAX_TRACKS,
};
...
impl<W: Write + Seek> MuxWriter<W> {
    pub fn new(
        ...
        if tracks.len() > usize::from(MAX_TRACKS) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "track table exceeds MAX_TRACKS",
            ));
        }
    ...
    pub fn write_packet(
        ...
        let block = encode_packet_block(&packet, crc32c)?;
        ...
        if self.header.cue_interval_packets > 0
            && self
                .packet_count
                .is_multiple_of(self.header.cue_interval_packets as u64)
        {
    ...
    fn write_periodic_cue(&mut self) -> io::Result<()> {
        ...
        let crc32c = self.header.block_checksum_is_crc32c();
        write_all(&mut self.writer, &encode_cue_block(&cue, crc32c)?)?;
        self.cue_count += 1;
    ...
    pub fn finalize(mut self) -> io::Result<W> {
        ...
        let crc32c = self.header.block_checksum_is_crc32c();
        write_all(&mut self.writer, &encode_index_block(&index, crc32c)?)?;
        self.writer.flush()?;
