...
use libsrs_contract::{CodecType, Packet};
    ...
    while let Some(pkt) = demux.next_packet()? {
        if pkt.packet.header.track_id == 1 {
            ...
            if let Some((writer, sample_rate)) = audio_writer.as_mut() {
                let frame = libsrs_audio::decode_frame(
                    *sample_rate,
                    pkt.packet.header.sequence as u32,
                    &pkt.packet.payload,
                )?;
...
fn run_native_import(pipeline: &TranscodePipeline, input: &Path, output: &Path) -> Result<usize> {
    let mut native = NativeImportTranscoder::new(output)?;
    pipeline.import_to_native(input, &mut native)
}
...
impl NativeImportTranscoder {
    fn new(output: &Path) -> Result<Self> {
        ...
        let mux = MuxWriter::new(File::create(output)?, FileHeader::new(1, 8), tracks)?;
        Ok(Self {
            mux: Some(mux),
            frame_idx: 0,
        })
...
impl NativeTranscoder for NativeImportTranscoder {
    fn transcode_packet(&mut self, packet: Packet) -> Result<()> {
        ...
        for (idx, slot) in data.iter_mut().enumerate() {
            *slot = packet.data[idx % packet.data.len()];
        }
        let frame = VideoFrame {
            width: 16,
            height: 16,
            frame_index: self.frame_idx as u32,
            frame_type: FrameType::I,
            data,
        };
        let payload = libsrs_video::encode_frame(&frame)?;
        let pts = packet
            .pts
            .map_or(self.frame_idx * 3_000, |ts| ts.ticks.max(0) as u64);
