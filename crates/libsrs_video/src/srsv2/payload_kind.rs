...
pub enum Srsv2PayloadKind {
    ...
    Intra,
    /// `FR2\\x02` … `FR2\\x09` — forward/inter predicted; experimental **B** / extended inter revisions (**10**–**11**, **13**–**31** except **12**) are classified here as **predicted** for mux/index policy (not keyframes) even when logical frame type is **B** — see `docs/video_bitstream_v2.md` for decode support per revision.
    Predicted,
...
///
/// - `FR2\\x01` / `FR2\\x03` / `FR2\\x07` / `FR2\\x29` → [`Srsv2PayloadKind::Intra`] (rev 3/7 use entropy residuals; rev 7 adds block `qp_delta`; rev **29** is strict intra residual ContextV1)
/// - Forward/inter and experimental **B** revisions (**2**, **4**–**11**, **13**–**31**) → [`Srsv2PayloadKind::Predicted`] for mux/index policy (includes **P** rev **19**/**20**/**23**/**25**/**27**/**28**/**30**, **B** rev **16**/**18**/**24**, and reserved **B** rev **21**/**22**/**26**/**31** — **decode** may still return `Unsupported` for some of these; see `docs/video_bitstream_v2.md`)
...
mod classify_tests {
    ...
    #[test]
    fn fr2_rev29_is_intra_kind() {
        assert_eq!(
            classify_srsv2_payload(&[b'F', b'R', b'2', 29]).unwrap(),
            Srsv2PayloadKind::Intra
        );
    }
    ...
    #[test]
    fn fr2_rev31_is_predicted_mux_policy_like_other_b_revisions() {
        assert_eq!(
            classify_srsv2_payload(&[b'F', b'R', b'2', 31]).unwrap(),
            Srsv2PayloadKind::Predicted
        );
    }
}
